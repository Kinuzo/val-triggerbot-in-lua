...
function script_defaults(config)
obs.obs_data_set_default_double(config, "delayfofo", 0.0)
end
...
obs.obs_properties_add_float_slider(binds, "delayfofo", "Delay", 0.0, 150.0, 0.25)
...
function script_update(config)
    delayfofo = obs.obs_data_get_double(config, "delayfofo") / 180.0
...
function script_tick(seconds)
    if initialize_window() and ffi.C.GetAsyncKeyState(bindzinha) > 0 then
        ...
        for y = -3, 4, 1 do
            for x = -1, 2, 1 do
                local pixelzin = ffi.C.GetPixel(captura, posicao + x, altura + y + distancia + delayfofo)
